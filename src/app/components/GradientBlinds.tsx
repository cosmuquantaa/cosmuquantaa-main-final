...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  float dn = d / r;
  // Changed to create brightness only at cursor, darkness elsewhere
  float spot = (1.0 - pow(clamp(dn, 0.0, 1.0), 1.0 / max(uSpotlightSoftness, 0.1))) * uSpotlightOpacity;
  vec3 cir = vec3(spot);
  float stripe = fract(uvMod.x * max(uBlindCount, 1.0));
  if (uShineFlip > 0.5) stripe = 1.0 - stripe;
    vec3 ran = vec3(stripe);

    // Modified to multiply base by spotlight for cursor-only lighting effect
    vec3 col = (base * cir) - (ran * cir * 0.5);
